...
class StationInputForm(forms.ModelForm):
    ...
    latitude =  forms.DecimalField(max_digits=10, decimal_places=8, max_value=90, min_value=-90)
    longitude = forms.DecimalField(max_digits=11, decimal_places=8, max_value=180, min_value=-180)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "country" in self.data:
            ...
            cities = [city_data['name'] for city_data in CSVData.CITIES if int(city_data['country_id']) == self.instance.country.id]
            choices = [(city, city) for city in cities]
            self.fields["city"]._set_choices(choices)
